...
update_fedora() {
	...
	rsync -avSH --delete-delay --delay-updates --exclude '**/source/**' --exclude '**/armhfp/**' --exclude '**/aarch64/**' --exclude '**/debug/**' --exclude '**/test/**' --exclude '**/testing/**' --delete-excluded --bwlimit=25M rsync://$UPSTREAM/fedora-enchilada0/ /data/mirror/fedora/
	sleep 10
	flock -u "$lock_fd"
}
